...
pub fn transformInverse(dst: *[6]f32, src: *const [6]f32) bool {
    ...
    inv[0] = @as(f32, @floatCast(t[3] * invdet));
    inv[2] = @as(f32, @floatCast(-t[2] * invdet));
    inv[4] = @as(f32, @floatCast((t[2] * t[5] - t[3] * t[4]) * invdet));
    inv[1] = @as(f32, @floatCast(-t[1] * invdet));
    inv[3] = @as(f32, @floatCast(t[0] * invdet));
    inv[5] = @as(f32, @floatCast((t[1] * t[4] - t[0] * t[5]) * invdet));
    return true;
}
