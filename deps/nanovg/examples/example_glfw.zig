...
pub fn main() !void {
    ...
    while (c.glfwWindowShouldClose(window) == c.GLFW_FALSE) {
        ...
        const dt = t - prevt;
        prevt = t;
        fps.update(@as(f32, @floatCast(dt)));
        ...
        win_width = @as(i32, @intFromFloat(@as(f32, @floatFromInt(win_width)) / scale));
        win_height = @as(i32, @intFromFloat(@as(f32, @floatFromInt(win_height)) / scale));
        ...
        _ = dt;
        vg.beginFrame(@as(f32, @floatFromInt(win_width)), @as(f32, @floatFromInt(win_height)), pxRatio);

        demo.draw(vg, @as(f32, @floatCast(mx)), @as(f32, @floatCast(my)), @as(f32, @floatFromInt(win_width)), @as(f32, @floatFromInt(win_height)), @as(f32, @floatCast(t)), blowup);
        fps.draw(vg, 5, 5);
