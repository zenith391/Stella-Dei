...
fn dotGridGradient(perm: [256]GridPoint, ix: i32, iy: i32, x: f32, y: f32) f32 {
	const dx = x - @intToFloat(f32, ix);
	const dy = y - @intToFloat(f32, iy);

	var point = perm[@intCast(usize, iy * 16 + ix)];
...
// Improved Perlin Noise - https://mrl.cs.nyu.edu/~perlin/noise/
...
fn grad(hash: u8, x: f32, y: f32, z: f32) f32 {
	...
	const v = if (h < 4) y else
		if (h == 12 or h == 14) x else z;
	return if (h&1 == 0) u else -u + if(h&2 == 0) v else -v;
}

const permutation = [256]u8 { 151,160,137,91,90,15,
